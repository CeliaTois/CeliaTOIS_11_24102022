...
def get_max_number_of_places(club_points, number_of_competition_places):
    max_places = 12
    if (number_of_competition_places < club_points and
            number_of_competition_places < 12):
...
def book(competition, club):
    ...
    if (foundClub and foundCompetition and
            foundCompetition['date'] > current_datetime):
        max_places = get_max_number_of_places(
            int(foundClub['points']), int(foundCompetition['numberOfPlaces']))
        return render_template(
            'booking.html',
            club=foundClub,
...
def purchasePlaces():
    ...
    max_places = get_max_number_of_places(
        int(club['points']), int(competition['numberOfPlaces']))
